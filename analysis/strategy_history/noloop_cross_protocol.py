    ...
    def build_market_data_dict(self, row_group: pd.DataFrame, strategy: Dict) -> Optional[Dict]:
        ...
        try:
            return {
                ...
                'collateral_ratio_token1': leg_1A['collateral_ratio'],              # Direct access - fails if missing
                'liquidation_threshold_token1': leg_1A['liquidation_threshold'],    # Direct access - fails if missing

                # Leg 2A - borrowing
                'rate_token2': leg_2A['borrow_total_apr'],
                'price_token2': leg_2A['price_usd'],
                'borrow_fee_token2': leg_2A['borrow_fee'],                          # Direct access - fails if missing
