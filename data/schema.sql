...
-- Table 5: position_snapshots
-- Stores lazy/optional snapshots of position state over time
CREATE TABLE IF NOT EXISTS position_snapshots (
    snapshot_id TEXT PRIMARY KEY,
    position_id TEXT NOT NULL,
    snapshot_timestamp TIMESTAMP NOT NULL,

    -- Current Rates at Snapshot Time (forward-looking)
    lend_base_apr_1A DECIMAL(10, 6),
    lend_reward_apr_1A DECIMAL(10, 6),
    borrow_base_apr_2A DECIMAL(10, 6),
    borrow_reward_apr_2A DECIMAL(10, 6),
    lend_base_apr_2B DECIMAL(10, 6),
    lend_reward_apr_2B DECIMAL(10, 6),
    borrow_base_apr_3B DECIMAL(10, 6),  -- NULL for unlevered
    borrow_reward_apr_3B DECIMAL(10, 6),  -- NULL for unlevered

    -- Current Prices at Snapshot Time (leg-level for Step 6)
    ...
    -- Current Collateral Ratios at Snapshot Time
    current_collateral_ratio_1A DECIMAL(10, 6),
    current_collateral_ratio_2B DECIMAL(10, 6),

    -- PnL Breakdown (leg-level for Step 6)
    ...
    -- Risk Metrics (calculated from rates/prices - Step 7)
    health_factor_1A_calc DECIMAL(10, 4),
    health_factor_2B_calc DECIMAL(10, 4),
    distance_to_liq_1A_calc DECIMAL(10, 6),
    distance_to_liq_2B_calc DECIMAL(10, 6),
    ltv_1A_calc DECIMAL(10, 6),
    ltv_2B_calc DECIMAL(10, 6),
    liquidation_price_1A_calc DECIMAL(20, 10),
    liquidation_price_2B_calc DECIMAL(20, 10),

    -- Phase 2: Protocol-sourced risk metrics (from SDK/API)
    health_factor_1A_protocol DECIMAL(10, 4),
    health_factor_2B_protocol DECIMAL(10, 4),
    liquidation_price_1A_protocol DECIMAL(20, 10),
    liquidation_price_2B_protocol DECIMAL(20, 10),
    ltv_1A_protocol DECIMAL(10, 6),
    ltv_2B_protocol DECIMAL(10, 6),
    risk_data_source TEXT,  -- 'calculated', 'protocol', or 'both'

    -- Collateralization Validation
    collateral_ratio_change_1A DECIMAL(10, 6),  -- % change from entry
    collateral_ratio_change_2B DECIMAL(10, 6),  -- % change from entry
    collateral_warning BOOLEAN DEFAULT FALSE,
...
CREATE INDEX IF NOT EXISTS idx_snapshots_pos_time ON position_snapshots(position_id, snapshot_timestamp);